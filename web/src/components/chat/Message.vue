    ...
    <div class='message-wrapper flex flex-col'>
        <div :class='["flex gap-2 group", fromSelf ? "flex-row-reverse self-end" : "self-start"]'>
            ...
            <div class='w-10 flex-shrink-0 self-center transition duration-100'>
                <div :class='["items-center justify-end", error ? "flex" : "hidden group-hover:flex"]'>
                    <button @click='resendMessage' v-if='error' class='w-8 h-8 flex items-center justify-center bg-slate-700 rounded-full text-indigo-200 hover:bg-slate-600 transition duration-75'>
                        <RetryIcon :size='25'/>
                    </button>
                </div>
            </div>
