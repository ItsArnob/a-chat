...
describe("Passport Bearer Strategy", () => {
    ...
    it("should validate user", async () => {
        ...
        const user: UserNoProfile = { id: "1", passwordHash: "passwordHash", username }
    ...
    it("should throw invalid username or password with incorrect credentials", async() => {
        ...
        jest.spyOn(authService, "validateUser").mockRejectedValue(new UnauthorizedException(message));
    ...
    it("should throw internalServerError when unknown error occurres", async() => {
        jest.spyOn(authService, "validateUser").mockRejectedValue(new Error("haha unknown error!"));

        await expect(localStrategy.validate(username, password)).rejects.toThrow(InternalServerErrorException);
        expect(authService.validateUser).toBeCalledTimes(1);
        expect(authService.validateUser).toBeCalledWith(username, password);      
    })
})