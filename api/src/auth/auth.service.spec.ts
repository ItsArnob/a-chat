...
describe("AuthService", () => {
    ...
    const user: UserNoProfile = { id: "1", passwordHash: '$2b$10$Wz9VGNz0oJPtAv7akD3HYeVjscV49dPWVOADO8DiDN48iSvE60VjG', username: "test" }
    const userWithProfile: User = { ...user, profile: { relations: [{ id: "2", status: RelationStatus.Friend }]}};
    const currentTime = 123456789;
    const idleTimeout = 2;

    let session: Omit<Session, 'id'> & { _id: string } = { _id: "1", expiresAt: new Date(), token: "validToken", userId: "1", name: "Chrome on Linux" };
    let sessionOfNonExistentUser: Omit<Session, 'id'> & { _id: string } = { _id: "2", expiresAt: new Date(), token: "validTokenButNoUser", userId: "3", name: "Chrome on Linux" };
    ...
    beforeEach(async () => {
        const moduleRef = await Test.createTestingModule({
            ...
            if(token === MONGODB_PROVIDER) {
                return {
                    sessions: {
                        findOne: jest.fn(async(query: Filter<SessionDoc>, opts: FindOptions<Document>) => {
                            if(query?.token === session.token) return Promise.resolve(session);
                            if (query?.token === sessionOfNonExistentUser.token) return Promise.resolve(sessionOfNonExistentUser);
                            return null;
                        }),
                        insertOne: jest.fn(),
                        updateOne: jest.fn(),
            ...
            if(token === UsersService) {
                return {
                    findOneNoProfileByName: jest.fn(async(username: string) => Promise.resolve(username === user.username ? user : null)),
                    findOneById: jest.fn(async(userid: string) => Promise.resolve(userid === userWithProfile.id ? userWithProfile : null )),
                    findOneNoProfileById: jest.fn(async(userid: string) => Promise.resolve(userid === user.id ? user : null )),
                }
            }
    ...
    describe("validateUser", () => {
        ...
        it("should successfully validate user with correct credential", async() => {
            await expect(authService.validateUser("test", "qweasdzxc")).resolves.toBe(user);
        });

        it("should throw user not found with invalid username", async() => {
            const res = async () => await authService.validateUser("test1", "qweasdzxc");
        ...
        it("should throw incorrect password error with incorrect password", async() => {
            ...
            const res = async () => await authService.validateUser("test", "qweasdzxc1");
            await expect(res).rejects.toThrow(UnauthorizedException);
    ...
    describe("validateToken", () => {
        ...
        it("should successfully validate token (should not return the profile object)", async() => {
            const userInfoWithSession: User & { sessionId: string, sessionName: string } = { ...user, sessionId: "aSessionId", sessionName: "aa" };
            const token = "token";
            const mockFindOne = jest.spyOn(authService, "findOneByToken").mockResolvedValue(userInfoWithSession);
            const mockTouchSession = jest.spyOn(authService, "touchSession");

            await expect(authService.validateToken(token)).resolves.toBe(userInfoWithSession);
            
            expect(mockFindOne).toBeCalledWith(token, undefined);
            
            expect(mockFindOne).toBeCalledTimes(1);
            
            expect(mockTouchSession).toBeCalledWith(userInfoWithSession.sessionId);
        ...
        it("should successfully validate token (should return the profile object)", async () => {
            const userInfoWithSession: User & { sessionId: string, sessionName: string } = { ...user, sessionId: "aSessionId", sessionName: "aa", profile: { relations: [{id: "awe", status: RelationStatus.Friend}]} };
            const token = "token";
            const mockFindOne = jest.spyOn(authService, "findOneByToken").mockResolvedValue(userInfoWithSession);
            const mockTouchSession = jest.spyOn(authService, "touchSession");

            await expect(authService.validateToken(token, true)).resolves.toBe(userInfoWithSession);
        ...
        it("should throw invalid session token when non-existent session is provided", async() => {
            ...
            const fn = async () => await authService.validateToken(invalidToken);

            const mockFindOne = jest.spyOn(authService, "findOneByToken").mockResolvedValue('session-not-found');
            const mockTouchSession = jest.spyOn(authService, "touchSession");
        ...
        it("should throw invalid session token when user doesn't exist but the session does", async() => {
            const token = "token";
            const mockFindOne = jest.spyOn(authService, "findOneByToken").mockResolvedValue("user-not-found");
            const mockTouchSession = jest.spyOn(authService, "touchSession");
    ...
    describe("findOneByToken", () => {
        ...
        it("should find a user by validToken (should not return the profile object)", async()=> {
            ...
            await expect(authService.findOneByToken(validToken)).resolves.toStrictEqual({...user, sessionId: session._id, sessionName: session.name });
            expect(mongo.sessions.findOne).toBeCalledWith({ token: validToken }, { projection: sessionProjection });
        ...
        it("should find a user by validToken (should return the profile object)", async()=> {
            await expect(authService.findOneByToken(validToken, true)).resolves.toStrictEqual({ ...userWithProfile, sessionId: session._id, sessionName: session.name });
            expect(mongo.sessions.findOne).toBeCalledWith({ token: validToken }, { projection: sessionProjection });
        ...
        it("should return session-not-found", async () => {
            await expect(authService.findOneByToken(nonExistentToken)).resolves.toBe("session-not-found");
            expect(mongo.sessions.findOne).toBeCalledWith({ token: nonExistentToken }, { projection: sessionProjection });
            expect(usersService.findOneNoProfileById).toBeCalledTimes(0);
            expect(usersService.findOneById).toBeCalledTimes(0);
        })
        ...
        it("should return user-not-found", async () => {
            ...
            await expect(authService.findOneByToken(validTokenNonExistentUser)).resolves.toBe("user-not-found");
            expect(mongo.sessions.findOne).toBeCalledWith({ token: validTokenNonExistentUser }, { projection: sessionProjection });
            expect(usersService.findOneNoProfileById).toBeCalledWith(sessionOfNonExistentUser.userId);
            expect(usersService.findOneNoProfileById).toBeCalledTimes(1);
            expect(usersService.findOneById).toBeCalledTimes(0);
        })
    ...
    describe("createSession", () => {
        ...
        it("should create session (with no name)", async () => {
            ...
            await expect(authService.createSession(user.id)).resolves.toStrictEqual({ id: sessionId, token: sessionToken });
        ...
        it("should create session (with name)", async () => {
            ...
            await expect(authService.createSession(user.id, sessionName)).resolves.toStrictEqual({ id: sessionId, token: sessionToken });
            expect(mongo.sessions.insertOne).toBeCalledWith({
                userId: user.id,
                _id: sessionId,
                token: sessionToken,
